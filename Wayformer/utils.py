...
class _TrajectoryVAE(nn.Module):
    """Standalone VAE for ego-local trajectory encoding/decoding.

    Architecture (reverse-engineered from checkpoint):
      encoder: 60→128→128→16  → mu(8) + logvar(8)
      decoder: 8→128→128→60  → [30, 2]
    """

    def __init__(self, latent_dim: int = 8, traj_len: int = 30, feature_dim: int = 2):
        super().__init__()
        self.latent_dim = latent_dim
        in_dim = traj_len * feature_dim  # 60
    ...
    def encode(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        """x: [B, traj_len, 2] → mu, logvar: [B, latent_dim]"""
    ...
    def decode(self, z: Tensor) -> Tensor:
        """z: [B, latent_dim] → [B, traj_len, 2]"""
        out = self.decoder(z)
        return out.reshape(z.shape[0], -1, 2)
...
class Decoder(nn.Module):
    ...
    def __init__(self,config:config):
        ...
        # A single learned query represents the one trajectory/skill mode.
        # Device placement is intentionally left to Lightning/model.to(...).
        self.trajectory_query = nn.Parameter(
        ...
        weight_path = getattr(config, 'vae_weight_path', None)
        if not weight_path:
            raise ValueError("Single-mode skill regression requires vae_weight_path")

        latent_dim = getattr(config, 'vae_latent_dim', 8)
        self.regressor = nn.Linear(config.HIDDEN_SIZE, latent_dim)
        self._vae_latent_dim = latent_dim
        self.vae = self._load_frozen_vae(
            weight_path,
            latent_dim=latent_dim,
            traj_len=config.pred_horizon,
        )

        nn.init.normal_(self.trajectory_query, mean=0.0, std=0.01)

    def _load_frozen_vae(self, weight_path: str, latent_dim: int, traj_len: int):
        """Load the pretrained TrajectoryVAE and freeze all parameters."""
        vae = _TrajectoryVAE(latent_dim=latent_dim, traj_len=traj_len, feature_dim=2)
        state_dict = torch.load(weight_path, map_location='cpu')
        vae.load_state_dict(state_dict)
        vae.eval()
        for p in vae.parameters():
            p.requires_grad = False
        self._vae_weight_path = weight_path
        return vae

    def forward(self, memory: Tensor, mask: Tensor = None) -> Tensor:
        """Regress one VAE latent code per scene.

        Args:
            memory: Encoded scene tokens, [B, N, D].
            mask: Invalid memory positions, [B, N].

        Returns:
            Predicted skill latent, [B, latent_dim].
        """
    ...
    def decode(self, z: Tensor) -> Tensor:
        """Decode skill latents into ego-local xy trajectories."""
        return self.vae.decode(z)

    def train(self, mode: bool = True):
        """Keep the frozen VAE in evaluation mode."""
        super().train(mode)
        self.vae.eval()
        return self
