...
class data_processor(object):
    ...
    def __init__(self, scenario, iteration: int = 0):
        ...
        if not 0 <= iteration < num_iterations:
            raise IndexError(
                f"iteration must be in [0, {num_iterations - 1}], "
                f"got {iteration}"
            )
    ...
    def get_map_features(self) -> np.ndarray:
        ...
        """
        ego_state = self.scenario.get_ego_state_at_iteration(self.iteration)
        ...
        traffic_light_data = self.scenario.get_traffic_light_status_at_iteration(
            self.iteration
        )
        vectors = list()
        polyline_span = list()
    ...
    def collect_agents(self):
       ...
       """
       initial_ego_state = self.scenario.get_ego_state_at_iteration(
           self.iteration
       )
    ...
    def build_mapping(self,debug=False):
       ...
       mapping = {
           ...
           'iteration': self.iteration,
           'timestamp_us': self.scenario.get_time_point(
               self.iteration
           ).time_us,
           'ego_global_pose': np.asarray(
               [
...
class NuplanDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        for sc in tqdm(scenarios, desc=f"Building {mode} scenarios"):
            try:
                mapping = data_processor(sc).build_mapping()
                self.ex_list.append(compress(pickle.dumps(mapping)))
                processed_count += 1
