...
from Wayformer.wayformer_config import (
...
def train_model(model: WayformerPL):
  ...
  logger = TensorBoardLogger(
      save_dir=f'{config.output_dir}', name=config.exp_name
  )
  callbacks = [
      LearningRateMonitor(),
      ModelCheckpoint(
  ...
  trainer = pl.Trainer(
      ...
      precision=32,
      detect_anomaly=False,  # 改为 False 防卡顿
  )
  try:
    # 设置 num_workers=2 启动场景提取，并开启 reuse_cache=True
    train_set = NuplanDataset(
        config=config,
        mode='train',
        num_workers=2,
        reuse_cache=True,
    ...
    val_set = NuplanDataset(
        config=config, mode='val', num_workers=2, reuse_cache=True, val_ratio=0.2
    )
    print(
  ...
  train_loader = DataLoader(
      ...
      shuffle=True,
      num_workers=min(2, config.data_workers),
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--config_dir',
      type=str,
      default='/home/thsxw/real-Skillformer-master/real-Skillformer-master/config/wayformer.1.json',
  )
  ...
  torch.autograd.set_detect_anomaly(False)  # 改为 False
